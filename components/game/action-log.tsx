...
const getStanceIcon = (msg: string) => {
  ...
  if (msg.includes("[Sneaky]") || msg.includes("chose Sneaky")) return <Eye className="w-3 h-3 text-sneaky" />
...
const parseDiceRoll = (msg: string): { dice: ParsedDice | null; cleanMsg: string } => {
...
const cleanMessage = (msg: string) => {
  return msg.replace(/\[(Defensive|Sneaky|Aggressive)\]\s*/g, "").replace(/\[DICE:[^\]]+\]\s*/g, "")
}

type MessageType = "player1" | "player2" | "system"

const getMessageType = (msg: string, player1Name: string, player2Name: string): MessageType => {
...
function DiceRollDisplay({ dice }: { dice: ParsedDice }) {
  return (
    ...
    >
      ...
      <span className={cn("font-bold", dice.success ? "text-heal" : "text-destructive")}>
        {dice.success ? "✓" : "✗"}
      </span>
    </div>
  )
...
export function ActionLog({ messages, player1Name = "Player 1", player2Name = "Player 2" }: ActionLogProps) {
