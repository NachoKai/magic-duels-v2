    ...
    (stance: Stance) => {
      if (gameMode === "cpu") {
        ...
        if (result === "win") {
          ...
          } = applySpell(gameState.player2, gameState.player1, cpuSpell);
          ...
          setGameState((prev) => ({
            ...
            player2Stance: cpuStance,
            phase:
              newP1.health <= 0 || newP2.health <= 0 ? "gameover" : "stance",
            stanceWinner: null,
            turn: prev.turn + 1,
            lastAction: [...prev.lastAction, ...messages],
          }));
  ...
  const handleSpellSelect = useCallback(
    (spell: Spell) => {
      ...
      const {
        ...
        target,
        messages: spellMessages,
      } = applySpell(casterState, targetState, spell);
      ...
      const newP1 =
        winner === 1 ? { ...caster, ...p1Result.player } : p1Result.player;
      const newP2 =
        winner === 1 ? p2Result.player : { ...caster, ...p2Result.player };
        ...
        <div className="bg-card rounded-xl border border-border p-6">
          {gameState.phase === "gameover" ? (
            ...
            <StanceSelector
              ...
              playerName={
                gameMode === "local" && gameState.waitingForPlayer2Stance
                  ? "Player 2"
                  : "Player 1"
              }
            />
            ...
            <SpellSelector
              stance={
                gameState.stanceWinner === 1
                  ? gameState.player1Stance!
